...
/**
 * @author Rajesh This class methods are used to Perform operations in
 *         WebApplication
 */
public class SeleniumHelper {
	...
	public String getLocatorFromObjecRepo(String objectRepoPath, String locator, String... values) {
		...
		try (BufferedReader reader = new BufferedReader(
				new FileReader(System.getProperty("user.dir") + objectRepoPath))) {
			...
			logger.error("IOException:: getLocatorFromObjecRepo:: " + e.getMessage());
		} catch (Exception e) {
			logger.error("Exception:: getLocatorFromObjecRepo:: " + e.getMessage());
		}
		return loc;
	}
	...
	protected WebElement getElement(String strElement) {
		...
		WebDriverWait wait = new WebDriverWait(driver, Duration.ofSeconds(PropertyFileLoader.getConfigInstance().getWebDriverWait()));
		try {
		...
		try {
			...
			if (isExists(element)) {
				if (isEnabled(element)) {
					...
					logger.info("Entered text: " + strText);
				}
			}
		} catch (Exception e) {
	...
	protected WebDriver getDriver() {
		return WebAppDriverManager.getDriver();
	}
	...
	public void imageScreenShot(String imageName) {
		try {
			...
			// PropertyFileLoader.getConfigInstance().getConfigValue("actualpath")+
			// imageName +".png");

			File destinationPath = new File(System.getProperty("user.dir")
					+ PropertyFileLoader.getConfigInstance().getConfigValue("actualpath") + imageName + ".png");
			destinationPath.getParentFile().mkdirs();
			destinationPath.createNewFile();
			Files.copy(sourcePath, destinationPath);
			// System.out.println("before reporter screenshot : " + destinationPath + "," +
			// sourcePath);
			
		//	Reporter.addScreenCaptureFromPath(destinationPath.toString());

		} catch (IOException e) {
			System.out.println("Into Imparision Result");
		}
